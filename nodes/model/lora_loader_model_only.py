...
class LoraLoaderModelOnly(LoraLoader):
    ...
    DESCRIPTION = "仅将 LoRA 应用到扩散模型（MODEL），行为与原生 LoraLoaderModelOnly 一致；同时输出该 LoRA 的触发词（Trigger Words）、描述（Description）与文件名（不含路径和扩展名）；节点上的信息图标可查看/编辑 LoRA 元数据与自定义备注。"
    ...
    RETURN_NAMES = ("MODEL", "trigger_words", "description", "lora_stem")
    OUTPUT_TOOLTIPS = ("修改后的扩散模型", "LoRA 触发词", "LoRA 描述", "LoRA 文件名（不含路径和扩展名）")
