...
# Apache-2.0), vendored via ylchen333/ComfyUI-VOSR2, sized to load the official
# `dinov2_vitl14` checkpoint directly from a local safetensors file.
#
...
# and a stack of pre-norm transformer blocks with LayerScale.
#
# sfnodes 改动（对齐 TE-Speed-VOSR2 的加速项）：`interpolate_pos_encoding` 结果
# 按 (h, w, dtype) 记忆化 —— 视频/分块推理里同一瓦片几何会被反复请求，bicubic
# resize 每次重算是纯浪费；缓存上限 8 条防分辨率扫描撑爆。
import math
...
# comfy.ops.disable_weight_init.{Conv2d,Linear,LayerNorm} 与 torch.nn 同 state_dict
# 键/同 forward，只跳过随机初始化；让 DINOv2 编码器可被 ComfyUI 的精度/低显存机制
# 管理（strict 加载与数值不受影响）。
...
class DinoVisionTransformer(nn.Module):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4.0):
        ...
        # 瓦片几何在同一分辨率下反复出现（视频逐帧/多瓦片），记忆化 bicubic
        # pos-embed resize；上限 8 条，分辨率扫描不会无限增长。
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        cache_key = (h, w, previous_dtype)
        cached = self._pos_embed_cache.get(cache_key)
        if cached is not None:
            return cached.to(device=x.device, dtype=previous_dtype)
        ...
        if len(self._pos_embed_cache) >= _POS_EMBED_CACHE_MAX:
            self._pos_embed_cache.clear()
        self._pos_embed_cache[cache_key] = result.detach()
        return result
