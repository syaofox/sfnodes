# Vendored from VOSR's models/lightningdit.py via ylchen333/ComfyUI-VOSR2
# (Apache-2.0; built on facebookresearch/DiT and willisma/SiT, by Jingfeng Yao,
# HUST-VL). Trimmed for VOSR2 v1:
# - timm's PatchEmbed/Mlp replaced by the small local equivalents (zero new deps)
# - torch.compile decorators dropped (ComfyUI 平台/加速器不一定有 Triton)
# - only forward_flexible's path is kept (fixed-size forward / sin-cos pos-embed
#   helpers are dead code for this RoPE-conditioned one-step model)
#
# sfnodes 改动（对齐 TE-Speed-VOSR2 的加速项）：
# - `_vosr_attention`：优先用 ComfyUI 的 SageAttention 适配器（自带版本/布局
#   兼容与回退），torch.compile 图内自动退回 SDPA（sage 自定义 CUDA op 不保证
#   在所有 Windows/Triton 组合上图兼容）
# - `_get_dynamic_rope` 结果按 (hw_seq_len, device, dtype) 记忆化：视频逐帧/分块
#   推理瓦片几何固定，arange/einsum/cos/sin 每次重算纯属浪费
import logging
import math
...
try:
    # 复用 ComfyUI 已测试的 SageAttention 适配器（版本/张量布局/回退都在内部处理）
    from comfy.ldm.modules.attention import (
        SAGE_ATTENTION_IS_AVAILABLE,
        attention_sage,
    )
except Exception:  # pragma: no cover - 老版本 ComfyUI 无该符号时静默退回 SDPA
...
def vosr_attention_backend():
    """返回本模块 DiT 注意力实际使用的后端名（日志/诊断用）。"""
    return "sage" if SAGE_ATTENTION_IS_AVAILABLE and attention_sage is not None else "sdpa"


def _vosr_attention(q, k, v, heads):
    """DiT 注意力：有 SageAttention 就用，否则 SDPA。

    q/k/v 已是 ComfyUI 的 [batch, heads, tokens, head_dim] 布局。
    """
    global _SAGE_LOGGED
    use_sage = SAGE_ATTENTION_IS_AVAILABLE and not torch.compiler.is_compiling()
    if use_sage and attention_sage is not None:
        if not _SAGE_LOGGED:
            logging.info("[sfnodes/VOSR2] DiT attention backend: SageAttention")
            _SAGE_LOGGED = True
        return attention_sage(
            q, k, v, heads, skip_reshape=True, skip_output_reshape=True,
        )
    if not _SAGE_LOGGED and SAGE_ATTENTION_IS_AVAILABLE:
        logging.info("[sfnodes/VOSR2] DiT attention backend: PyTorch SDPA (compile path)")
        _SAGE_LOGGED = True
...
class Attention(nn.Module):
    ...
    def forward(self, x, rope=None):
        ...
        x = _vosr_attention(q, k, v, self.num_heads)
...
class LightningDiT(nn.Module):
    ...
    def __init__(
        ...
        # 视频/分块推理瓦片几何固定，动态 RoPE 张量按 (seq_len, device, dtype) 记忆化
        self._dynamic_rope_cache = {}
    ...
    def _get_dynamic_rope(self, hw_seq_len, device, dtype):
        ...
        cache_key = (int(hw_seq_len), device_key, dtype)
        cached = self._dynamic_rope_cache.get(cache_key)
        ...
        if len(self._dynamic_rope_cache) >= _ROPE_CACHE_MAX:
            self._dynamic_rope_cache.clear()
        self._dynamic_rope_cache[cache_key] = dynamic_rope_fn
