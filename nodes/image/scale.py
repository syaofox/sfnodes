...
class LoadImageScaled(LoadImage):
    @classmethod
    def INPUT_TYPES(cls):
        base_inputs = super().INPUT_TYPES()
    ...
    def load_image(self, image, upscale_method, total_pixels, limit):
        ...
        result_dict = image_scaler_by_pixels.execute(
            image_output, upscale_method, total_pixels, limit, mask
        )

        return result_dict


class LoadImageFromSubfolder(LoadImage):
    @classmethod
    def INPUT_TYPES(cls):
...
class LoadImageByMtime(LoadImage):
    @classmethod
    def INPUT_TYPES(cls):
        input_dir = folder_paths.get_input_directory()
        files = []
