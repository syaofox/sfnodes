...
class SFPromptReader:
    DESCRIPTION = (
        "SF Prompt Reader - 读取用 ComfyUI（或 Automatic1111 / Forge）生成的图片或视频，"
        "恢复保存在其中的正向提示词。无预览，只输出文本。\n\n"
        "支持 PNG（tEXt chunks：ComfyUI workflow JSON 或 A1111 'parameters'）与 "
        "MP4 / WebM / MKV 视频（VideoHelperSuite 等保存节点写入的元数据）。"
        "把文件拖到节点上、点 Upload 上传、或从文件下拉中选择——选中的瞬间"
        "即开始读取，运行前就能看到提示词。输出为 STRING，可直接接入 "
        "CLIPTextEncode 或其他文本输入复用。\n\n"
        "文件列表默认来自 input/ 目录；点文件行左侧的 IN/OUT 按钮可切换到 "
        "output/ 目录（读取用 output 节点保存的图片/视频）。在 output 模式下"
        "上传或拖放文件会自动切回 input/（上传的文件落在那里）。\n\n"
        "文件没有内嵌提示词（JPG、截图、或元数据丢失的文件）时，输出一句说明文本"
        "并显示在节点上，下游接线不受影响。\n\n"
        ...
        "可选 'filename' 输入可接入文件名（例如 Load Image SF 的 filename 输出）"
        "——接线期间节点忽略自己的选择器，改读该文件的提示词；在节点上选择、上传或"
        "拖放文件即接管并自动断开该连线。"
    )

    @classmethod
    def INPUT_TYPES(cls):
        # Walk input/ recursively so subfolder media files are listed too.
        # Forward slashes in the paths so folder_paths.get_annotated_filepath
        # resolves them correctly cross-platform. Mirrors node_load_image.py.
        ...
        return {
            "required": {
                "image": (sorted(files), {"image_upload": True, "tooltip": "要读取提示词的文件。上传、拖放或选择用 ComfyUI / Automatic1111 / Forge 生成的 PNG，或含元数据的 MP4 / WebM / MKV 视频。选中的瞬间即开始读取。"}),
            },
            "optional": {
                ...
                "filename": ("STRING", {"forceInput": True, "tooltip": "可选。把文件名接入此处（例如 Load Image SF 的 filename 输出）以自动读取该文件的提示词。接线期间节点忽略自己的选择器；在节点上选择、上传或拖放文件即接管并断开连线。"}),
            },
        }
