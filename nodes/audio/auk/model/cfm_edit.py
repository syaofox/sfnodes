...
from ..model.utils import exists, lens_to_mask
from .memory_utils import fuse_layers, euler_final
...
class CFMEdit(nn.Module):
    ...
    def build_cond_inputs(messages_batch, processor):
        ...
        if has_mm:
            media = [c for msgs in messages_batch for m in msgs for c in m["content"]
                     if isinstance(c, dict) and c.get("type") in ("audio", "image", "video")]
            if all(c["type"] == "audio" and isinstance(c.get("audio"), np.ndarray)
                   and "audio_start" not in c and "audio_end" not in c for c in media):
                # ComfyUI supplies mono 16 kHz arrays; no file/video decoder is needed.
                mm_audios = [c["audio"] for c in media]
                if any(audio.ndim != 1 for audio in mm_audios):
                    raise ValueError("Qwen audio input must be mono.")
                mm_images = mm_videos = None
            else:
    ...
    def encode_text(self, cond_inputs, device) -> torch.Tensor:
        ...
        if self.low_memory:
            hidden = fuse_layers(all_hidden_states, self.layer_weights, self.layer_scale)
            return hidden.to(output_device), attention_mask.to(output_device).bool()
    ...
    def sample(
        ...
        *,
        ...
        def fn(t, x):
            ...
            if self.sequential_cfg:
                common = dict(x=x, text=text_embeds, time=t, mask=target_mask,
                              c_mask=context_mask, ref=ref_latent, ref_mask=ref_mask, cache=False)
                v_cond = self.transformer(**common, drop_audio_cond=False, drop_text=False)
                v_uncond = self.transformer(**common, drop_audio_cond=True, drop_text=True)
