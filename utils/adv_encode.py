...
def divide_length(word_ids, weights):
    ...
    sums[0] = 1
    weights = [
        [_norm_mag(w, sums[id]) if id != 0 else 1.0 for w, id in zip(x, y)]
        for x, y in zip(weights, word_ids)
    ]
    return weights
...
def shift_mean_weight(word_ids, weights):
    delta = 1 - np.mean(
        [w for x, y in zip(weights, word_ids) for w, id in zip(x, y) if id != 0]
    )
    weights = [
        [w if id == 0 else w + delta for w, id in zip(x, y)]
        for x, y in zip(weights, word_ids)
    ]
    return weights
...
def scale_to_norm(weights, word_ids, w_max):
    ...
    w_max = min(top, w_max)
    weights = [
        [w_max if id == 0 else (w / top) * w_max for w, id in zip(x, y)]
        for x, y in zip(weights, word_ids)
    ]
    return weights
...
def mask_word_id(tokens, word_ids, target_id, mask_token):
    new_tokens = [
        [mask_token if wid == target_id else t for t, wid in zip(x, y)]
        for x, y in zip(tokens, word_ids)
    ]
...
def from_masked(tokens, weights, word_ids, base_emb, length, encode_func, m_token=266):
    ...
    weight_dict = dict(
        (id, w) for id, w in zip(wids, np.array(weights).reshape(-1)[inds]) if w != 1.0
    )
...
def mask_inds(tokens, inds, mask_token):
    ...
    inds_set = set(inds)
    new_tokens = [
        [mask_token if i * clip_len + j in inds_set else t for j, t in enumerate(x)]
...
def down_weight(tokens, weights, word_ids, base_emb, length, encode_func, m_token=266):
    ...
    w_mix = torch.tensor(w_mix, dtype=embs.dtype, device=embs.device).reshape(
        (-1, 1, 1)
    )
...
def recover_dist(base_emb, weighted_emb):
    fixed_std = (base_emb.std() / weighted_emb.std()) * (
        weighted_emb - weighted_emb.mean()
    )
...
def advanced_encode_from_tokens(
    ...
    if weight_interpretation == "comfy":
        weighted_tokens = [
            [(t, w) for t, w in zip(x, y)] for x, y in zip(tokens, weights)
        ]
    ...
    if weight_interpretation == "compel":
        pos_tokens = [
            [(t, w) if w >= 1.0 else (t, 1.0) for t, w in zip(x, y)]
            for x, y in zip(tokens, weights)
        ]
        weighted_emb, _ = encode_func(pos_tokens)
        weighted_emb, _, pooled = down_weight(
            pos_tokens, weights, word_ids, weighted_emb, length, encode_func
        )

    if weight_interpretation == "comfy++":
        weighted_emb, tokens_down, _ = down_weight(
            unweighted_tokens, weights, word_ids, base_emb, length, encode_func
        )
...
def prepareXL(embs_l, embs_g, pooled, clip_balance):
    l_w = 1 - max(0, clip_balance - 0.5) * 2
    g_w = 1 - max(0, 0.5 - clip_balance) * 2
...
def advanced_encode(
    ...
    if isinstance(
        clip.cond_stage_model, (SDXLClipModel, SDXLRefinerClipModel, SDXLClipG)
    ):
        embs_l = None
        embs_g = None
        pooled = None
...
def advanced_encode_XL(
    ...
    return prepareXL(
        embs_l.expand((-1, repeat_l, -1)),
        embs_g.expand((-1, repeat_g, -1)),
        pooled,
        clip_balance,
    )
