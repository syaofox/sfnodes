...
class CropperInsightFace(object):
    def __init__(self, **kwargs) -> None:
        ...
        self.landmark_runner = LandmarkRunner(
            ckpt_path=os.path.join(
                folder_paths.models_dir, "liveportrait", "landmark.onnx"
            ),
        ...
        self.face_analysis_wrapper.prepare(
            ctx_id=device_id, det_size=(512, 512), det_thresh=detection_threshold
        )
    ...
    def crop_single_image(
        ...
        src_face = self.face_analysis_wrapper.get(
            img_rgb, flag_do_landmark_2d_106=True, direction=direction
        )
        ...
        lmk = recon_ret["pts"]
        ret_dct["lmk_source"] = lmk

        recon_crop_ret = self.landmark_runner.run(image_crop, ret_dct["pt_crop"])
        lmk_crop = recon_crop_ret["pts"]
        ret_dct["lmk_crop"] = lmk_crop

        # print("InsightFace")
        # print("len of lmk: ", len(lmk))
        # print("len of pts: ", len(ret_dct['pt_crop']))

        return ret_dct, image_crop
...
class CropperMediaPipe(object):
    def __init__(self, **kwargs) -> None:
        ...
        if provider != "torch_gpu":
            ...
            self.landmark_runner = LandmarkRunnerTorch(
                ckpt_path=os.path.join(
                    folder_paths.models_dir, "liveportrait", "landmark_model.pth"
                ),
                onnx_provider=provider,
                device_id=device_id,
            )
    ...
    def crop_single_image(
        ...
        if face_result is None:
            ret_dct = {}
            cropped_image_256 = None
            print("CropperMediaPipe no face detected")
            return ret_dct, cropped_image_256
...
class CropperFaceAlignment(object):
    def __init__(self, **kwargs) -> None:
        ...
        if "blazeface" in face_detector:
            face_detector_kwargs = {
                "back_model": face_detector == "blazeface_back_camera"
            }
    ...
    def crop_single_image(
        ...
        # print("faceAlignment")
        # print("len of lmk: ", len(lmk))
        # print("len of pt_crop: ", len(ret_dct['pt_crop']))
