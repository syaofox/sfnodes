// ==========================================================================
// sf_pause_latent_ui.js - SFPauseLatent 节点体 UI（单个 DOM widget）
// ==========================================================================
//
// 与 sf_pause_image_ui.js 同构（类前缀 sf-pl-）：Pause/Pass 切换、状态行、
// 两行按钮（Regenerate/Continue + Copy/Save Disk/Save Output/Open）、预览图
// （object-fit:contain）、尺寸行。预览显示的是 image 输入（VAEDecode 结果），
// 与 image 闸门同为彩色 RGB。
//
...
import { getState } from "./sf_pause_latent_lib.js";
...
function injectCSS() {
    ...
    s.id = "sf-pl-css";
    s.textContent = `
.sf-pl-root { display:flex; flex-direction:column; flex:1 1 0; min-height:0;
  box-sizing:border-box; padding:6px; gap:6px; font:12px sans-serif; color:#ddd;
  overflow:hidden; }
.sf-pl-toggle { display:flex; background:rgba(0,0,0,0.25); border-radius:6px; padding:2px; gap:2px; flex:0 0 auto; }
.sf-pl-seg { flex:1 1 0; text-align:center; padding:4px 0; border-radius:5px; cursor:pointer;
  color:rgba(255,255,255,0.6); user-select:none; border:1px solid transparent; }
.sf-pl-seg.active { background:${"var(--sf-acc, #f66744)"}; color:#fff; border-color:${"var(--sf-acc, #f66744)"}; }
.sf-pl-seg:not(.active):hover { border-color:${"var(--sf-acc, #f66744)"}; color:#ddd; }
.sf-pl-status { flex:0 0 auto; font-size:11px; color:rgba(255,255,255,0.7); min-height:14px; text-align:center; }
.sf-pl-btns { display:flex; gap:6px; flex:0 0 auto; }
.sf-pl-btn { flex:1 1 0; min-width:0; height:26px; line-height:24px; border-radius:4px;
...
.sf-pl-btn:hover:not(:disabled) { border-color:${"var(--sf-acc, #f66744)"}; color:#fff; }
.sf-pl-btn.primary:not(:disabled) { background:${"var(--sf-acc, #f66744)"}; border-color:${"var(--sf-acc, #f66744)"}; color:#fff; }
.sf-pl-btn.primary:hover:not(:disabled) { background:#ff8a5e; border-color:#ff8a5e; }
.sf-pl-btn:disabled { opacity:0.45; cursor:default; }
.sf-pl-preview { flex:1 1 0; min-height:0; position:relative; background:#1d1d1d;
  border:1px solid #333; border-radius:4px; overflow:hidden; }
.sf-pl-img { position:absolute; inset:0; width:100%; height:100%; object-fit:contain; display:none; }
.sf-pl-empty { position:absolute; inset:0; display:flex; align-items:center; justify-content:center;
  text-align:center; color:#777; font-size:11px; padding:8px; box-sizing:border-box; }
.sf-pl-dims { flex:0 0 auto; text-align:center; font-size:10px; color:#aaa;
...
// node._sfPauseLatentEls，供 renderPause / showFrame 使用。
...
export function renderPause(node) {
    const els = node._sfPauseLatentEls;
    if (!els) return;
    const s = getState(node);
    const paused = s.gate === "pause";
    const hasSnap = !!node._sfPauseLatentHasSnapshot;  // 仅运行时（见 lib 注释）
    ...
    if (node._sfPauseLatentFlash) {
        els.status.textContent = node._sfPauseLatentFlash;
    } else if (node._sfPauseLatentBusy) {
        els.status.textContent = node._sfPauseLatentBusy;
    } else if (!paused) {
...
export function showFrame(node, frame) {
    ...
    img.onload = () => {
        ...
        // 状态、不弄脏工作流
        node._sfPauseLatentHasSnapshot = true;
        renderPause(node);
    };
    img.onerror = () => {
        ...
        dims.textContent = "";
        node._sfPauseLatentHasSnapshot = false;
        renderPause(node);
    };
    img.src = url;
}
