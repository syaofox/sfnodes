// ==========================================================================
// sf_note.js - SF 注释（复刻孤海注释 GoohaiNote.js，纯前端节点）
// ==========================================================================
//
// 原版是无后端纯前端扩展，本文件按原版架构 1:1 移植（行为一致）：
// LiteGraph.registerNodeType 注册画布节点 + canvas 绘制多行文本（CJK 换行
// 引擎见 sf_note_lib.js，调用处以 measure 注入替代 ctx，其余逐义一致）+
// 双击 DOM 编辑器（含工具条全套）+ 链接点击打开 + 固定模式 + 右键菜单 +
// drawNode/processMouseDown/getNodeOnPos 全局补丁 + document 鼠标监听 +
// Nodes 2.0 Vue 覆盖层双击穿透。
//
// 相对原版的改动（最小适配集）：
// - import 改绝对路径 "/scripts/app.js"；扩展名 "sfnodes.Note"
//  （check_web_imports.py B2/B3）；样式 id 改 sf-note-* 前缀
...
const NOTE_TYPE = "SF Note";
const NOTE_TITLE = "SF Note";
...
class TextEditorNode extends TextEditorBaseNode {
    ...
    drawMultilineText(ctx, text, maxWidth, lineHeight) {
        ...
        const lines = wrapCharList(ctx.measureText.bind(ctx), chars, maxWidth);
...
TextEditorNode.type = NOTE_TYPE;
TextEditorNode.title = NOTE_TITLE;
...
// ── 全局补丁（原版同款行为；once 守卫防重复加载叠包）──
function patchGlobalsOnce() {
    if (typeof LGraphCanvas !== "undefined" && !LGraphCanvas.prototype._sfNotePatched) {
        LGraphCanvas.prototype._sfNotePatched = true;
    ...
    if (typeof LGraph !== "undefined" && !LGraph.prototype._sfNotePatched) {
        LGraph.prototype._sfNotePatched = true;
...
// Nodes 2.0 HTML VUE覆盖层双击穿透（原版同款）
function installVueDblClickRelayOnce() {
    if (installVueDblClickRelayOnce.done) return;
    installVueDblClickRelayOnce.done = true;
...
function installDblClickCursorFixOnce() {
    if (document.getElementById("sf-note-dblclick-style")) return;
    const st = document.createElement("style");
    st.id = "sf-note-dblclick-style";
    st.textContent = `
...
function canvasCenterPos() {
    try {
        ...
        const w = cv?.clientWidth || globalThis.innerWidth || 1280;
        const h = cv?.clientHeight || globalThis.innerHeight || 800;
        const ds = canvas?.ds || { offset: [0, 0], scale: 1 };
        return [(w / 2 - ds.offset[0]) / ds.scale, (h / 2 - ds.offset[1]) / ds.scale];
    } catch {
        return [0, 0];
    }
}
...
async function addSFNote() {
    ...
    try {
        ...
        if (cmd) {
            await cmd.execute("Comfy.AddNode", { type: NOTE_TYPE });
