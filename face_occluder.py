...
class OccluderLoader:
    ...
    def get_occluder(self, model_choice='xseg_2'):
        ...
        print(f"已加载XSEG模型: {self.selected_model} - {get_model_description(self.selected_model)}")
...
class GeneratePreciseFaceMask:
    ...
    def generate_mask(self, occluder, input_image, mask_threshold, post_process, grow, grow_percent, grow_tapered, blur, fill):
        ...
        for i in range(steps):
            mask, processed_img = self._process_single_image(input_image[i], face_occluder_model, mask_threshold, post_process, grow, grow_percent, grow_tapered, blur, fill)
    ...
    def _process_single_image(self, img, face_occluder_model, mask_threshold, post_process, grow, grow_percent, grow_tapered, blur, fill):
        """处理单张图像"""
        face = tensor2np(img)
        if face is None:
        ...
        if post_process == 'gaussian_blur':
            occlusion_mask = cv2.GaussianBlur(occlusion_mask.clip(0, 1), (0, 0), 5).clip(0, 1)
