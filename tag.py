...
def fixed_get_imports(filename: str | os.PathLike) -> list[str]:
    ...
    try:
        ...
        logger.debug("No flash_attn import to remove")
        pass
    return imports
...
class Tagger:
    ...
    def tag_image(
        ...
        inputs = (
            processor(text=prompt, images=image, return_tensors="pt", do_rescale=False)
            .to(dtype)
            .to(device)
        )
    ...
    def start_tag(
        ...
        if not os.path.exists(model_path):
            logger.info(f"Downloading Lumina model to: {model_path}")
            from huggingface_hub import snapshot_download

            snapshot_download(
                repo_id=hg_model, local_dir=model_path, local_dir_use_symlinks=False
            )
...
class FluxCLIPTextEncode:
    ...
    def encode(self, clip, caption, guidance):
        ...
        empty_output = clip.encode_from_tokens(
            empty_tokens, return_pooled=True, return_dict=True
        )
...
class CaptionAnalyzer:
    ...
    def analyze(self, analyze, subject_index, add_is_text=True, **kwargs):
        ...
        for item in analyze.split(","):
            try:
                if ":" not in item:
                    analyze_dict[previous_key.strip()] = (
                        f"{analyze_dict[previous_key.strip()]}, {item.strip()}"
                    )
                else:
        ...
        for key, flag in kwargs.items():
            if flag and key in analyze_dict:
                if (
                    subject_index == 0
                    or len(analyze_dict[key].split(";")) < subject_index
                ):
                    analyze_result = analyze_dict[key]
                else:
                    analyze_result = (
                        analyze_dict[key].split(";")[subject_index - 1].strip()
                    )

                analyze_result = (
                    analyze_result.replace("NA", "unknown")
                    .replace("n/a", "unknown")
                    .replace("N/A", "unknown")
                ...
                if analyze_result != "unknown":
                    if add_is_text:
                        selected_analyze.append(
                            f"{key.replace('_', ' ')} is {analyze_result}"
                        )
                    else:
...
NODE_CLASS_MAPPINGS = {
    "fox_Tagger": Tagger,
    "fox_SaveTags": SaveTags,
    "fox_Flux_CLIPTextEncode": FluxCLIPTextEncode,
    "fox_Caption_Analyzer": CaptionAnalyzer,
}

# A dictionary that contains the friendly/humanly readable titles for the nodes
NODE_DISPLAY_NAME_MAPPINGS = {
    "fox_Tagger": "🐾fox Tagger",
    "fox_SaveTags": "🐾fox Save Tags",
    "fox_Flux_CLIPTextEncode": "🐾fox Flux Clip Text Encode",
    "fox_Caption_Analyzer": "🐾fox Caption Analyzer (Beta)",
}
