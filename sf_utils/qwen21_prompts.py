"""Qwen Image 2.1 官方 Prompt Enhancer 系统提示词（原文内嵌，非本项目撰写）。

来源（Qwen 官方发布，2026-09，抓取日期 2026-09-22）：
- T2I: https://huggingface.co/Qwen/Qwen-Image-2.1-PE-T2I/blob/main/system_prompt.txt
       （与 https://github.com/QwenLM/Qwen-Image-2.1/blob/main/prompt_rewrite/prompts/system_prompt_t2i.txt
       逐字一致，已 diff 校验）
- I2I: https://github.com/QwenLM/Qwen-Image-2.1/blob/main/prompt_rewrite/prompts/system_prompt_edit.txt
       （同 Qwen/Qwen-Image-2.1-PE-I2I 发布内容）

许可：Qwen Research License Agreement（仅研究与评估用途，商用需另行授权）。文本逐字保留，
仅供 SFQwenImage21PromptEnhancer 的「本地官方PE」模式使用——官方 PE 模型为 Qwen3.5-VL 9B
微调，答案契约（JSON 输出）是训练的一部分，偏离官方提示词会静默漂移（官方 README 明示）。
通用/API 模式用 qwen21_enhance.py 中的自写协议，不使用本文件。
"""

OFFICIAL_T2I_SYSTEM_PROMPT = r"""# Image Prompt Rewriting Expert
...
OFFICIAL_EDIT_SYSTEM_PROMPT = r"""# Edit Prompt Enhancer — General (v2, 精简版)
