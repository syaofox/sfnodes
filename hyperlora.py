...
def str_field(
    ...
    return name, ("STRING", {"default": default, "multiline": multiline})
...
def inputs_def(required: Iterable = [], optional: Iterable = []) -> Dict[str, Dict]:
    return {
        "required": {k: v for k, v in required},
        "optional": {k: v for k, v in optional},
    }
...
class HyperLoRALoadCharLoRANode:
    ...
    def INPUT_TYPES(cls):
        return inputs_def(
            required=[enum_field("charname", options=list_chars("hyper_lora/chars"))]
        )
    ...
    def execute(self, charname):
        filename = os.path.join(
            folder_paths.models_dir, "hyper_lora/chars", f"{charname}.safetensors"
        )
        assert os.path.isfile(filename), f"LoRA文件未找到: {filename}"
        lora = load_file(filename)
        # 查找同名图片
        img_pattern = os.path.join(
            folder_paths.models_dir, "hyper_lora/chars", f"{charname}_??.png"
        )
                ...
                if images_tensor[i].shape != ref_shape:
                    adjusted = (
                        comfy.utils.common_upscale(
                            images_tensor[i].movedim(-1, 0).unsqueeze(0),
                            ref_shape[1],
                            ref_shape[0],
                            "lanczos",
                            "center",
                    ...
                    logger.info(f"图片 {i + 1} 已调整为 {ref_shape}")
        ...
        if images is not None and hasattr(images, "shape") and images.shape[0] > 0:
            ...
            for i in range(1, images.shape[0]):
                if images[i].shape != ref_shape:
                    ...
                    adjusted = (
                        comfy.utils.common_upscale(
                            images[i].movedim(-1, 0).unsqueeze(0),
                            ref_shape[1],
                            ref_shape[0],
                            "lanczos",
                            "center",
