...
- **画布多选宽度对齐**（platform §11）：`sf_canvas_align` 画布背景 `getCanvasMenuItems` 子菜单（Widest/Narrowest/First Selected 仅改宽度，高度不变），选中集兼容 Object/Array/Map/Set 四形态 + `is_selected` 回退，最小宽钳制 `computeSize()[0]`，撤销走 `beforeChange/afterChange`。
