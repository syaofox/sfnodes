...
- **SFImageInterrogator 三模态可选**（lora §38）：`image/video/audio` 均 `optional` 对齐原生 `Generate Text`，无图时纯文本生成（不插视觉占位符）；`video` 为 `IMAGE` batch 24→1FPS 抽帧逐帧缩放、多帧按 `Picture N:` 前缀；`user_prompt` 由 `forceInput` 改末尾 `STRING multiline` 文本框可连可填，追加末尾保 `widgets_values` 不错位；`_scale_image None→[]` 防御。
