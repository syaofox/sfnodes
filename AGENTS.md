...
- **SFCharacterSelect 角色三分镜单选**（text §47）：单节点多角色切换——角色卡内三联排（脸部特写/半身像/全身像）+ hover 三图大预览，输出提示词 STRING + 三路独立 IMAGE（单分镜缺失仅该路 1×1 黑占位）；角色库 `character_` 前缀 JSON（内置 data/characters + user/sfnodes/characters 同名覆盖，包内仅格式骨架无二进制），新双路由 `/api/sfnodes/characters*`（库名前缀隔离 + commonpath 钳位）；张量/占位/落盘复用 id_clothing 同包 import；隐藏双真源 SFCharacterState（单选取首）+ SFCharacterPrompt（手改草稿，切换即清）。
