...
- `NODE_CLASS_MAPPINGS`: 键 `"SF<ClassName>"`，值为类本身（全部键带 SF 前缀、与 DISPLAY 字典键一致；新增一律带前缀）
...
- **注册规范**（由 `tests/check_web_imports.py` 固化校验）：registerExtension 文件必须直接 `import { app } from "/scripts/app.js"`（禁相对路径）、扩展注册名 `sfnodes.*` 前缀、相对导入目标必须存在；多模块系列/共享库新增模块需加入该脚本 MODS 列表（单文件节点只走文件级规则，不必登记）
...
12. 新增/删除 py/js 文件同步 `doc/architecture.md` 条目；沉淀新经验按主题写入 `doc/experience/` 对应主题文件（取号 = `experience/README.md` 顶部「当前最大 §N」+ 1，并同步更新该数字与索引表；编号唯一、只增不复用）；**确属新类别且现有主题均不适配时可新建主题文件**（英文短名对齐节点族，标题只写主题，章节映射由 README 索引承担）
...
5. **后置复查**：按「Testing」跑三条回归；核对两注册字典键一致；`git diff` 对照开工基线逐文件复查，确保无错漏与无关改动；未经明确要求不执行 git commit/push（提交细则见「Git 提交规范」）
6. **硬约束查询**：涉及 ComfyUI API/节点注册/工具函数/JS Widget 的改动，先读本文件对应章节与 `doc/architecture.md` 逐文件细目，遵循既有约定
7. **不确定时**：务必追问，禁止猜测

## Git 提交规范

- **只提交任务范围内的改动**：`git add` 前用 `git status` / `git diff` 对照开工基线复查，无关文件不提交；提交前必须跑完「Testing」三条回归（仅文档/注释改动至少跑两条静态一致性检查）。
- **一个提交一个内聚变更**：功能代码与其测试、文档、依赖版本调整放同一提交；流程/规则类文档等独立主题另起 `docs(...)` 提交。
- **提交信息**：单行长中文摘要，格式 `<type>(<scope>): <内容>`；type 用 `feat` / `fix` / `docs` / `refactor` / `perf` / `chore` 等，scope 对齐功能组（audio/image/model/text/video/web/experience…）；细节用 `——` 引导、`+` 分隔，不写多行 body（历史提交同款风格）。
- **amend 规则**：**未 push** 的提交可用 `git commit --amend` 修正或补入同一逻辑的遗漏（依赖版本、测试/文档订正等），不产生「修正上一提交」的噪声提交，amend 后同步更新提交信息；**已 push** 的提交禁止 amend / force push，只能追加新提交。
- **部署副本不提交**：`/mnt/github/comfyui-docker/custom_nodes/sfnodes` 是运行实例的独立 git 仓库，只做文件同步，不在其上 commit（避免与工作副本分叉）；需要对齐时由用户 push 后在部署目录 fetch/reset 处理。
...
- 快速回归命令（文件自含断言，任一失败非零退出即停）：
  ...
  - 静态一致性：`python3 tests/check_web_imports.py`（web 模块导入/注册契约）+ `python3 tests/check_docs.py`（注册字典、§N 唯一性与引用、README 索引、architecture 文件覆盖）
