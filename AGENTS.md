...
- **SFPauseText 中⇄EN 翻译按钮**（text §74）：LLM API 翻译就地替换盒子文本（Continue/Keep 输出译文）；`sf_utils/translation.py` 纯函数（方向判定复用 `has_chinese_character`）+ `nodes/text/translate_routes.py` 代理 `POST /api/sfnodes/translate`（浏览器直连 LLM API 被 CORS 拦）；`sfnodes.Translate.{Provider,BaseUrl,Model,ApiKey}` 走 ComfyUI Settings（`type:"text"`），API key 不硬编码；`thinking:{"type":"disabled"}` 仅对 DeepSeek 端点携带（非标准字段，其他兼容端点会 400，按 base_url 判断）。
...
- **SCAIL-2 四节点复刻**（video §72）：`nodes/video/scail2.py` 四节点（Fit Video / Reference Pack / Reference SAM Builder / Simple Video）复用核心 `WanSCAILToVideo`·`SCAIL2ColoredMask`·`SAM3_VideoTrack`·`comfy.context_windows`，不重写推理；自定义类型串**保持原名** `SCAIL2_REFERENCE_PACK`、节点 ID 加 `SF` 前缀；纯逻辑（`4n+1` 帧数数学/32 对齐/Fit Video 尺寸/多主体拼图布局）单源 `sf_utils/scail2_easy.py`（无 torch），**原生 `_extract_mask_to_28ch` 只留薄包装禁内联 fallback**；`context_sampling` 走 `sf_utils/scail2_context.py` 给 `SCAILWanModel._forward` 打一次性幂等 patch（按窗口 `index_list` 切 driving/ref 28ch 蒙版，仅该模式触发）；前端 `web/sf_scail2.js` 中文单语，Reference Pack 是**数量驱动**动态槽（非 `sf_dynamic_slots` 连线驱动）、Simple Video `widgets_values` 位置敏感须 `onConfigure` 修复，显隐工具本模块内实现；可选 `tiled_decode` 开关（默认关，VAEDecodeTiled 降解码显存，编码侧不做 tiled 防时空伪影）。
...
- **PointsEditor 底图刷新**（patterns §68）：KJNodes 只认直接上游 `image|video` widget / `videopreview.videoEl` / 执行后 `imgs`，且重载不走 `onConnectionsChange` → 透传链（ImageScale/SFImageBatch/Any Switch/SFImageCropExpand）恒无底图；`web/sf_points_bg.js` 纯前端加「↻ 刷新底图」（无新节点），沿 `bg_image` 链解析（纯 lib `sf_points_bg_lib.js`：preview>videoEl>widget>file 优先级 + 防环）→ 复用 KJNodes `node.editor.processImage`（自身持久化 `properties.imgData`）零执行刷新；`file`(裁剪源) 用 `resize:false` 保坐标空间，特性检测降级。
