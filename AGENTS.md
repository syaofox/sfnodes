...
- **combo→输出槽类型前端改型**（patterns §41）：SFConvert Anything（复刻 easy convertAnything）后端 `RETURN_TYPES=(any_type,)` 静态声明，输出槽类型同步是纯前端职责——复用 `any_pack.js::setSlotType`（已 export，Vue reactive 数组必须**元素替换**非原地改）；挂点 = callback 包装（参数即新值）+ `onAfterGraphConfigured` 恢复（nodeCreated 早于值恢复）；None 输入直通（原件对 None 转 int 直接崩）；Function-eval 测试 strip 正则须同剥 `import` 语句与 `export ` 关键字；跨模块 import 的模块必须入 check_web_imports MODS。
