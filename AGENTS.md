...
- **画布菜单聚合**（platform §2.15）：本包画布背景右键唯一顶层入口 `📦 SF Menu`（`web/sf_canvas_menu.js`，聚合浏览器/工作流/便签/对齐（≥2 节点嵌套）/内存）；新增画布菜单项一律加进聚合器，禁止各文件自注分散入口；特性侧只 export 动作（前缀式 `export function/const`，花括号式会破坏 Function-eval 测试）。
