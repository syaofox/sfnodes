...
opt = it["optional"]
check("输入顺序 pose/couple/env", list(opt)[:3] == ["pose_preset", "couple_preset", "environment_preset"])
check("角度/距离/镜头顺序", list(opt)[5:8] == ["camera_angle_preset", "camera_distance_preset", "camera_lens_preset"])
for key in ("pose_preset", "couple_preset", "environment_preset", "lighting_preset", "style_preset", "camera_angle_preset", "camera_distance_preset", "camera_lens_preset"):
    opts = opt[key][0]
...
combined, pose, couple, env, light, style, angle, dist, cam = node.execute(
...
_, pose_only, _, env_only, _, _, _, _, _ = node.execute(
...
_segs = [s.strip().rstrip(".") for s in (couple, env, light, style, angle, dist, cam)]
check("combined 含全部部分", all(s in combined for s in _segs))
check("拼接顺序 couple < env", combined.index(_segs[0]) < combined.index(_segs[1]))
check("拼接顺序 angle < dist < lens", combined.index(_segs[4]) < combined.index(_segs[5]) < combined.index(_segs[6]))
comb_only, _, _, env_only_text, _, _, _, _, _ = node.execute("test subject", seed=42, pose_preset="回眸", couple_preset="禁用", environment_preset="现代地铁车厢")
...
_, pose_nsfw, _, _, _, _, _, _, _ = node.execute("x", seed=1, pose_preset="床上自慰")
check("NSFW 姿势命中", "masturbat" in pose_nsfw and "nude" in pose_nsfw)
_, _, couple_nsfw, _, _, _, _, _, _ = node.execute("x", seed=1, couple_preset="女女交叉体位")
check("NSFW 女女命中", "scissor" in couple_nsfw and "lesbian" in couple_nsfw)

# 3. english values are ignored
_, _, _, env2, _, _, _, _, _ = node.execute("x", seed=1, environment_preset="Ocean Sunrise")
check("英文预设名忽略", env2 == "")
_, _, _, env3, _, _, _, _, _ = node.execute("x", seed=1, environment_preset="Disabled")
check("英文 Disabled 忽略", env3 == "")
_, _, _, env4, _, _, _, _, _ = node.execute("x", seed=1, environment_preset="Random")
check("英文 Random 忽略", env4 == "")
_, _, _, env5, _, _, _, _, _ = node.execute("x", seed=1, environment_preset="None")
check("英文 None 忽略", env5 == "")

# 3b. pose english name ignored
_, pose_en, _, _, _, _, _, _, _ = node.execute("x", seed=1, pose_preset="Running")
check("姿势英文名忽略", pose_en == "")

# 4. deterministic weighted random
r1 = node.execute("", seed=100, pose_preset="随机", couple_preset="禁用", environment_preset="随机", lighting_preset="随机", style_preset="随机", camera_angle_preset="随机", camera_distance_preset="随机", camera_lens_preset="随机")
r2 = node.execute("", seed=100, pose_preset="随机", couple_preset="禁用", environment_preset="随机", lighting_preset="随机", style_preset="随机", camera_angle_preset="随机", camera_distance_preset="随机", camera_lens_preset="随机")
check("随机预设 seed 确定性", r1 == r2)
r3 = node.execute("", seed=100, pose_preset="随机", couple_preset="禁用", environment_preset="随机", lighting_preset="随机", style_preset="随机", camera_angle_preset="随机", camera_distance_preset="随机", camera_lens_preset="随机")
check("随机输出非空", all(r3[:2] + r3[3:]))
r4 = node.execute("", seed=200, pose_preset="随机", couple_preset="禁用", environment_preset="随机", lighting_preset="随机", style_preset="随机", camera_angle_preset="随机", camera_distance_preset="随机", camera_lens_preset="随机")
check("不同 seed 可能不同", r1 != r4 or r3 != r4)

# seed offsets: pose +1, couple +2, environment +3, angle +6, distance +7, lens +8
p_off = node._resolve_preset("Pose", "随机", 100 + 1)
c_off = node._resolve_preset("Couple Pose", "随机", 100 + 2)
e_off = node._resolve_preset("Environment", "随机", 100 + 3)
a_off = node._resolve_preset("Camera Angle", "随机", 100 + 6)
d_off = node._resolve_preset("Camera Distance", "随机", 100 + 7)
l_off = node._resolve_preset("Camera Lens", "随机", 100 + 8)
check("姿势随机偏移 seed+1", r1[1] == p_off)
check("环境随机偏移 seed+3", r1[3] == e_off)
check("角度随机偏移 seed+6", r1[6] == a_off)
check("距离随机偏移 seed+7", r1[7] == d_off)
check("镜头随机偏移 seed+8", r1[8] == l_off)
rc = node.execute("", seed=100, pose_preset="禁用", couple_preset="随机")
check("双人随机偏移 seed+2", rc[2] == c_off)

# 3c. enriched presets resolve correctly
_, _, _, env_new, _, _, _, _, _ = node.execute("x", seed=1, environment_preset="薰衣草田日落")
check("新增环境反查", "lavender" in env_new)
_, _, _, env_life, _, _, _, _, _ = node.execute("x", seed=1, environment_preset="樱花大道")
check("樱花大道反查", "cherry blossom" in env_life.lower())
_, _, _, _, light_life, _, _, _, _ = node.execute("x", seed=1, lighting_preset="日系柔和窗光")
check("日系窗光反查", "japanese" in light_life.lower() and "window light" in light_life.lower())
_, pose_new, _, _, _, _, _, _, _ = node.execute("x", seed=1, pose_preset="仰卧举腿")
check("新增姿势 NSFW 反查", "legs raised" in pose_new and "nude" in pose_new)
_, _, couple_new, _, _, _, _, _, _ = node.execute("x", seed=1, couple_preset="反向女上位")
check("新增双人 NSFW 反查", "reverse cowgirl" in couple_new)
_, _, _, _, _, _, angle_new, _, _ = node.execute("x", seed=1, camera_angle_preset="荷兰角")
check("角度反查命中", "dutch" in angle_new.lower())
_, _, _, _, _, _, angle_new2, _, _ = node.execute("x", seed=1, camera_angle_preset="过肩镜头")
check("过肩反查命中", "over-the-shoulder" in angle_new2.lower())
_, _, _, _, _, _, _, dist_new, _ = node.execute("x", seed=1, camera_distance_preset="大远景")
check("大远景反查命中", "extreme long shot" in dist_new.lower())
_, _, _, _, _, _, _, dist_new2, _ = node.execute("x", seed=1, camera_distance_preset="牛仔镜头")
...
_, _, _, _, _, _, _, _, _ = node.execute("", seed=1, camera_distance_preset="特写", camera_lens_preset="85mm经典人像")
...
_, p_m, c_m, _, _, _, _, _, _ = node.execute("x", seed=1, pose_preset="回眸", couple_preset="公主抱")
check("互斥兜底 pose 生效", "looking back over the shoulder" in p_m)
check("互斥兜底 couple 忽略", c_m == "")
_, p_m2, c_m2, _, _, _, _, _, _ = node.execute("x", seed=1, pose_preset="禁用", couple_preset="公主抱")
check("仅 couple 时生效", c_m2 != "" and "bridal carry" in c_m2 and p_m2 == "")
_, p_m3, _, _, _, _, _, _, _ = node.execute("x", seed=1, pose_preset="随机", couple_preset="公主抱")
