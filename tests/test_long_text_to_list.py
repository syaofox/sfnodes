...
# 2. 逗号
picked, lst, count = node.execute("x,y,z", ",", 1, False)
assert_eq(picked, "y", "comma")
assert_eq(count, 3)

# 3. 空分隔符 -> 单元素
picked, lst, count = node.execute("hello", "", 0, False)
assert_eq(lst, ["hello"], "empty delim list")
assert_eq(count, 1)
assert_eq(picked, "hello")

picked, lst, count = node.execute("", "", 0, False)
assert_eq(lst, [], "empty both list")
assert_eq(count, 0)
assert_eq(picked, "", "empty both picked")

# 4. 越界
picked, lst, count = node.execute("a,b", ",", 5, False)
assert_eq(picked, "", "oob picked")
assert_eq(count, 2)

picked, lst, count = node.execute("a,b", ",", 0, False)
assert_eq(picked, "a", "oob sanity")

# 5. \\t 制表
picked, lst, count = node.execute("a\tb\tc", "\\t", 1, False)
assert_eq(picked, "b", "tab")

# 6. None delimiter -> 同空分隔符
picked, lst, count = node.execute("hello world", None, 0, False)
...
# 7b. filter_empty
assert_eq(split_text("a\n\nb\n \nc", "\\n", True), ["a","b","c"], "filter_empty")
assert_eq(split_text("a,,b,,c", ",", True), ["a","b","c"], "filter_empty comma")
assert_eq(split_text("a\nb\nc", "\\n", False), ["a","b","c"], "no filter")
picked, lst, count = node.execute("a\n\nb\n \nc", "\\n", 1, True)
assert_eq(lst, ["a","b","c"], "node filter_empty list")
assert_eq(picked, "b", "node filter_empty picked")
assert_eq(count, 3)
picked, lst, count = node.execute("a\n\nb", "\\n", 0, False)
assert_eq(lst, ["a","","b"], "node no filter keep empty")
picked, lst, count = node.execute("a\n\nb", "\\n", 1, False)
assert_eq(picked, "", "node empty item")
picked, lst, count = node.execute("a\n\nb", "\\n", 1, True)
assert_eq(picked, "b", "node filter shifts index")
...
print("test_long_text_to_list: 30 assertions passed")
