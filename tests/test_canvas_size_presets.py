# sf_utils/canvas_size_presets 后端逻辑测试（Python 直接运行：python tests/test_canvas_size_presets.py）
...
# mock aiohttp.web + server.PromptServer（test_crop_expand_presets.py 范式）
...
tmp_user = tempfile.mkdtemp(prefix="sf_csp_test_")
...
spec = importlib.util.spec_from_file_location(
    "sfnodes.sf_utils.canvas_size_presets",
    os.path.join(root, "sf_utils", "canvas_size_presets.py"),
)
...
STORE = os.path.join(tmp_user, "sfnodes", "canvas_size_presets.json")
...
r = run(routes.handlers[("POST", PATH)](_FakeRequest({"name": "A", "w": 1.5, "h": 1})))
check("浮点 w 400", r.status == 400)
r = run(routes.handlers[("POST", PATH)](_FakeRequest({"name": "A", "w": 32769, "h": 1})))
...
# 4. POST 落盘 + GET
r = run(routes.handlers[("POST", PATH)](_FakeRequest({"name": "  Banner  ", "w": 1600, "h": 900})))
...
check("磁盘结构 {presets: [{name,w,h}]}", disk == {"presets": [{"name": "Banner", "w": 1600, "h": 900}]})
r = run(routes.handlers[("POST", PATH)](_FakeRequest({"name": "Tall", "w": 900, "h": 1600})))
check("追加第二条", r.status == 200)
r = run(routes.handlers[("GET", PATH)](None))
check("GET 返回两条保序", [p["name"] for p in r.data["presets"]] == ["Banner", "Tall"])

# 5. 同名覆盖（不追加）
r = run(routes.handlers[("POST", PATH)](_FakeRequest({"name": "Banner", "w": 1920, "h": 1080})))
check("同名覆盖成功", r.status == 200)
check("同名覆盖不追加", [p["name"] for p in mod.load_presets()] == ["Banner", "Tall"]
      and mod.load_presets()[0] == {"name": "Banner", "w": 1920, "h": 1080})
...
with open(STORE, "w", encoding="utf-8") as f:
    json.dump({"presets": [{"name": "Banner", "w": 1920, "h": 1080}, {"name": "C", "w": 512, "h": 512}]}, f, ensure_ascii=False)
...
mod._cache["sig"] = None
check("归一化过滤无效与重名",
      mod.load_presets() == [{"name": "A", "w": 3, "h": 1}, {"name": "D", "w": 2, "h": 2}])

# 9. _normalize_presets 接受裸列表
check("裸列表归一化", mod._normalize_presets([{"name": "X", "w": 5, "h": 4}]) == [{"name": "X", "w": 5, "h": 4}])

# 10. canvas_size.py 触发路由注册（文本断言：import 行存在）
src = open(os.path.join(root, "nodes", "utils", "canvas_size.py"), encoding="utf-8").read()
check("canvas_size.py 导入预设模块触发注册", "from ...sf_utils import canvas_size_presets" in src)
