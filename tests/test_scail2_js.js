...
check("uniform hides closed_loop", exported.isWidgetVisible(getW(advNode, "closed_loop")) === false);
getW(advNode, "context_schedule").value = "looped_uniform";
exported.updateSimpleVideoWidgets(advNode);
check("looped shows context_stride", exported.isWidgetVisible(getW(advNode, "context_stride")) === true);
check("looped shows closed_loop", exported.isWidgetVisible(getW(advNode, "closed_loop")) === true);
check("context_stride label", exported.SCAIL2_LABELS.SCAIL2SimpleVideo.context_stride === "窗口步幅");
check("closed_loop label", exported.SCAIL2_LABELS.SCAIL2SimpleVideo.closed_loop === "循环闭环");
...
check("reorder context_stride before closed_loop", orderNode.widgets.findIndex((w) => w.name === "context_stride") <
