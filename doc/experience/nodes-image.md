...
- **布局改版（2026-09，用户草图）**：面板收窄为两行（行1 Load Image/Browse，行2 Color/Custom/Reset），比例按钮改**画布区左侧竖列**（`RATIO_PRESETS_COL`，Free 置顶 + 7 预设，`LAYOUT.ratioColW/ratioColGap` 由图片区让宽），`MIN_NODE_WIDTH` 相应 460→320（原 row1 横排 6 键至 x≈350 的 400→460 约束不再存在）。
