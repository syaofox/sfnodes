...
- **下拉弹窗方向必须随内容重锚（2026-08）**：`sf_lora_stack_dropdown.js` 的 `place()`（能下就下，否则上翻钳 8）原本只在打开时与首次 `renderList()` 后调用——目录导航/面包屑/back/搜索触发 `renderList()` 但不重算定位，进大目录后高度增长（上限 60vh）而 top 仍是"向下"旧值 → 底边越出视口、`overflow:hidden` 裁掉内容不可见。修复：`place()` 移入 `renderList()` 末尾（提前 return 分支用 if/else 收口防漏）。**关键事实**：`max-height:60vh` 保证实际高度 ≤ 60% 视口 → 上翻（top=8）后弹窗恒完整可见，只需保证每次内容变化重跑 place。测试 `tests/test_lora_stack_dropdown_smoke.js`：注入可写 `offsetHeight` 模拟高度增长（向下 584 → 翻上 36 → back 翻回），fileRow 文本断言查 `createTextNode` 节点而非元素 `_text`。
