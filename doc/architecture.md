...
│   ├── sf_brush_mask*.js  # 画笔遮罩两模块（SFImageBrushMask：lib 纯几何/纯逻辑（显示坐标互算 localToImage/imageToLocal/落笔钳制 clampToImage/ensureMinSize MIN 420×320/命中 hitResizeCornerSE + 双端镜像 parseStroke/parseBrushData/buildBrushData，无依赖可 .mjs 直测）+ 主扩展（顶面板 Load/Browse/Clear/Undo/Eraser+Size/Opacity 滑块+双取色块 + 节点画布直接落笔（capture 拖拽 + 全局 mouseup 兜底）、状态存 properties.sfBrushMaskState 经 graphToPrompt 注入隐藏输入（仅 lean 字段：改 opacity/颜色不重跑）、源图经 CropAPI.uploadSrc 落盘 input/sfnodes_crop/（零新增路由）+ /view 恢复 + 粘贴/拖放/Browse 同 loadAndStoreImage 链路、computeSize 包装 + mousemove cursor 补写同 §44））
