...
│   └── 其余单节点 JS（text_replace/text_concatenate/simple_math（SFSimpleMath/SFSimpleMathCondition 动态槽位 + SFNumber number_type 切换联动 value 步进/精度——INT 整数档取整当前值、FLOAT 小数档、PERCENT ÷100 百分数书写档（切档 ×100/÷100 换算仅限 callback 路径，configure 恢复不换算），step2 精调步进一并覆盖，tests/test_number_js.js；SFNumber 输出单槽 any 化随档位输出真类型值，见 patterns.md §40）/loop_flow/any_pack/image_browser（SFLoadImageBrowser 弹窗浏览器：缩略图网格/面包屑/排序/删除，图片右键菜单——复制正向提示词走 /api/sfnodes/prompt_reader/extract、载入工作流经 loadWorkflowFromImageUrl+/view 原始字节）/lora_loader*/lora_loader_model_only/multi_lora_tree/image_compare/image_concatenate/regex_extract/prompt_batcher/empty_latent_ratio/krea2_*/seed/canvas_size/workflow_name/sf_combo_selector/sf_color_picker（COLOR widget 值归一为 hex：新版前端内置 ColorWidget 接管 COLOR 类型，nodeCreated/loadedGraphNode/configure 三时序归一旧数组值，tests/test_color_picker_js.js）/showcontrol/DisplayText/SFLogicSwitch/...）
