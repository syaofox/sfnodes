...
│   ├── characters.py     # 角色三分镜纯逻辑（单选收敛/草稿优先/face-half-full 分镜取值 + character_ 库过滤，落盘复用 id_clothing.resolve_thumbnail_path，SFCharacterSelect 用）
...
│   ├── sf_character*.js   # 角色三分镜两模块（SFCharacterSelect：lib 纯函数 + 主扩展（角色卡内三联排缩略图 + hover 三图大预览 + 可编辑提示词框 + 无有效选择自动首选，隐藏 SFCharacterState/SFCharacterPrompt 双真源，同文件注册路由 /api/sfnodes/characters*））
