...
│   ├── string.py        # 字符串工具（has_chinese_character/ split_text 复刻 ComfyUI_Lam 分割/ affix_list 列表前后缀，见 §36/§37）
│   ├── translation.py   # 翻译封装
...
│   └── 其余单节点 JS（text_replace/text_concatenate/simple_math/loop_flow/any_pack/image_browser（SFLoadImageBrowser 弹窗浏览器：缩略图网格/面包屑/排序/删除，图片右键菜单——复制正向提示词走 /api/sfnodes/prompt_reader/extract、载入工作流经 loadWorkflowFromImageUrl+/view 原始字节）/lora_loader*/lora_loader_model_only/multi_lora_tree/image_compare/image_concatenate/regex_extract/prompt_batcher/empty_latent_ratio/krea2_*/seed/canvas_size/workflow_name/sf_combo_selector/sf_color_picker（COLOR widget 值归一为 hex：新版前端内置 ColorWidget 接管 COLOR 类型，nodeCreated/loadedGraphNode/configure 三时序归一旧数组值，tests/test_color_picker_js.js）/showcontrol/DisplayText/SFLogicSwitch/...）
