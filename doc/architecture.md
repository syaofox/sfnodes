...
│   ├── sf_character*.js   # 角色多选图库两模块（SFCharacterSelect：lib 纯函数 + 主扩展（角色卡四格正方形预览 3+1 + shots 复选横条 + hover 大预览 + 可编辑提示词框 + 无有效选择自动首选，隐藏 SFCharacterState{"role",shots}/SFCharacterPrompt 双真源，同文件注册路由 /api/sfnodes/characters*））
