...
│   ├── sf_crop_expand*.js # 出界裁剪两模块（SFImageCropExpand：lib 纯几何（显示坐标/手柄命中/拖拽更新+比例约束/冻结快照防飘移/ensureMinSize 最小尺寸，无依赖可 .mjs 直测）+ 主扩展（节点画布直接绘制与交互、状态存 properties.sfCropExpandState 经 graphToPrompt 注入隐藏输入（outpaint 先例）、源图经 CropAPI.uploadSrc 落盘重载 /view 恢复、Ctrl+V 粘贴剪贴板图片（复用 sf_common installPasteHandler，加载走 loadAndStoreImage 同链路）、Load Image/Browse（复用 image_browser 弹窗 onPick 选择器模式）/Color 按钮 + Custom 比例弹窗复用 sf_popup 三关闭、布局=左侧竖列（Free 置顶 + 比例预设 + Custom/Reset/Color 收尾，节点顶直通画布区底）+ 底行（Load Image/Browse 按钮与信息文本同排，按钮 y 标记 bottom 运行时解析）、最小尺寸：computeSize 包装钳住双端拖拽 resize（onResize 是 legacy-only）+ 创建/恢复 clampNodeSize 兜底防按钮外溢 + window mousemove 补写 resizeDirection 修 cursor 视觉（原生命中区 15×15，防 hover 切换/第三方扩展重放清空）（加载图片不改节点大小）））
│   ├── sf_brush_mask*.js  # 画笔遮罩两模块（SFImageBrushMask：lib 纯几何/纯逻辑（CropExpand 同形 LAYOUT：左工具竖列 TOOL_COL 10 项 + 底行，显示坐标互算 localToImage/imageToLocal/落笔钳制 clampToImage/ensureMinSize MIN 420×320/命中 hitResizeCornerSE + 步进纯函数 stepBrushSize/stepOpacity（Size± 步长2/Opa± 步长5%）+ 双端镜像 parseStroke/parseBrushData/buildBrushData，无依赖可 .mjs 直测）+ 主扩展（左竖列 Brush/Erase/Clear/Undo/Size±/Opa±/双取色块（S±/O± 悬停滚轮快调，上滚增大/下滚减小） + 底行 Load/Browse 与信息文本同排（实时数值 Brush/Op/Strokes/Source，溢出截断）+ 节点画布直接落笔（capture 拖拽 + 全局 mouseup 兜底）、状态存 properties.sfBrushMaskState 经 graphToPrompt 注入隐藏输入（仅 lean 字段：改 opacity/颜色不重跑）、源图经 CropAPI.uploadSrc 落盘 input/sfnodes_crop/（零新增路由）+ /view 恢复 + 粘贴/拖放/Browse 同 loadAndStoreImage 链路、computeSize 包装 + mousemove cursor 补写同 §44））
