...
│   ├── utils/           # 工具：数学、显示、内存清理、分辨率、图像编辑、Qwen Edit 编码（qwen_edit.py：SFQwenEditTextEncode 复刻 ComfyUI-EditUtils QwenEditTextEncode——每图独立 ref_longest_edge/ref_crop/mask，pad 画布 vae_unit 对齐 + 主图初始 latent/noise_mask/pad_info + custom_output 全量中间产物；SFQwenEditOutputExtractor 拆包；裁剪 rope offsets；纯逻辑在 sf_utils/qwen_edit.py）、万能滑条（universal_slider.py：SFUniversalSlider 复刻孤海万能滑条——单 any 输出随 output_type 输出真类型 + IS_CHANGED 回显，前端 web/sf_universal_slider.js Canvas 大滑条，见 patterns.md §52）
