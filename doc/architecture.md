...
│   ├── image/           # 图片：加载（files.py：SFLoadImages / browser.py：SFLoadImageBrowser / load_images_path.py：SFLoadImagesPath 三源渐进式 / load_image_resize.py：SFLoadImageResize）、缩放（resize_image.py：SFImageResize wired 尺寸；scale.py：ImageResizePlus 增强（size_mode 双模式 width & height/total pixels，1024² MP 约定））、拼接（concatenate.py）、混合（blend.py）、切块（tile.py）、变换（transform.py/scale.py）、处理（processing.py）、对比（compare.py）、三点色彩匹配（color_match_points.py：SFImageColorMatchByPoints 亮度分位自动提取暗/灰/亮三点 → 逐通道三点分段线性 LUT）、LUT（lut.py）、仿色（imitation_hue.py）、批次索引（batch_index.py）、批次合并（batch.py：SFImageBatch 复刻 WAS Image Batch——多路 image_1..16 动态槽沿 batch 维 torch.cat 拼接，尺寸不一致抛错列出槽名，tests/test_image_batch.py）、可视化裁剪+贴回（crop.py）、出界裁剪/外绘预处理（crop_expand.py：SFImageCropExpand 复刻 YCNodes Load Image Crop Expand——节点上加载图片（Load Image/拖放），拖拽可出界裁剪框，输出填充色画布（交集=原像素/出界=纯色）+ 白=扩展区遮罩 + 宽高，源图持久化 input/sfnodes_crop/ 复用 crop.py 的 upload_src 路由，隐藏 SFCropExpandJson 单状态注入，见 experience/nodes-image.md §37）、外绘填充+贴回（outpaint.py）、RFMSR 超分（rfmsr_upscale.py：SFRFMSRUpscale）、图片闸门（pause_image.py）、latent 闸门（pause_latent.py：SFPauseLatent 分段采样中间暂停）、镜头切分（scene_split.py：SFImageSceneSplit 视频连续帧硬切/黑场/白闪/溶解检测→按 index 选段+全段 LIST）、精确保存（save_image_exact.py：SFSaveImageExact 精确文件名/可选覆盖/无计数后缀）、预览保存路由（preview_routes.py）
...
│   ├── sf_crop_expand*.js # 出界裁剪两模块（SFImageCropExpand：lib 纯几何（显示坐标/手柄命中/拖拽更新+比例约束/冻结快照防飘移，无依赖可 .mjs 直测）+ 主扩展（节点画布直接绘制与交互、状态存 properties.sfCropExpandState 经 graphToPrompt 注入隐藏输入（outpaint 先例）、源图经 CropAPI.uploadSrc 落盘重载 /view 恢复、Custom 比例弹窗复用 sf_popup 三关闭）
...
│   ├── sf_text_preset.js  # 持久化文本预设单模块（全局库 API 读写 + 工作流残留合并 + API 失败降级）
