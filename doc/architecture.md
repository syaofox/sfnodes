...
│   ├── utils/           # 工具：数学、显示、内存清理（memory_cleanup.py：VRAMCleanup/RAMCleanup 节点 + memory_routes.py：POST /api/sfnodes/memory/ram 画布菜单后端，复用 RAMCleanup，见 experience/platform.md §2.14）、分辨率、图像编辑、Qwen Edit 编码（qwen_edit.py：SFQwenEditTextEncode 复刻 ComfyUI-EditUtils QwenEditTextEncode——每图独立 ref_longest_edge/ref_crop/mask，pad 画布 vae_unit 对齐 + 主图初始 latent/noise_mask/pad_info + custom_output 全量中间产物；SFQwenEditOutputExtractor 拆包；裁剪 rope offsets；纯逻辑在 sf_utils/qwen_edit.py）、万能滑条（universal_slider.py：SFUniversalSlider 复刻孤海万能滑条——单 any 输出随 output_type 输出真类型 + IS_CHANGED 回显，前端 web/sf_universal_slider.js Canvas 大滑条，见 patterns.md §52）、布尔开关（boolean_switch.py：SFBooleanSwitch 复刻孤海布尔开关——单 BOOLEAN 输出直通，前端 web/sf_boolean_switch.js Canvas 大开关，见 patterns.md §53）、忽略多组（ignore_groups.py：SFIgnoreGroups 复刻孤海忽略多组——空壳 OUTPUT_NODE + 前端 web/sf_ignore_groups.js 编组开关面板，见 patterns.md §54）、文本便签（复刻孤海注释——无后端纯前端节点 web/sf_note.js，见 patterns.md §55）
...
│   ├── sf_canvas_align*.js # 画布多选尺寸对齐两模块（SF Align Width/Height/Size：lib 纯逻辑 getSelectedNodes/calcTargetWidth/calcTargetHeight/alignNodesWidth/alignNodesHeight/alignNodesSize 四形态兼容 + 最小尺寸钳制；主模块 export buildAlignMenuItems（≥2 节点才返回三入口，否则 []），供聚合菜单组装）
...
│   ├── sf_canvas_menu.js # 本包装布背景右键聚合菜单（唯一顶层入口 📦 SF Menu：浏览器/工作流/便签/对齐（≥2 节点嵌套 SF Align）/内存，动作全 import 各特性 export 零复制，tests/test_canvas_menu_js.js，见 experience/platform.md §2.15）
